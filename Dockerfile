...
RUN set -ex && apt update && apt install -y net-tools  \
    curl zip vim git telnet python3-pip jo \
    ...
    \
    && apt purge -y git uuid-runtime zip \
...
WORKDIR /etc/varnish
COPY ./config/processlogs.sh /etc/varnish/
...
RUN chmod +x /start.sh && chmod +x /etc/varnish/processlogs.sh

EXPOSE 8123

CMD sh -c /start.sh
HEALTHCHECK --interval=30s --timeout=30s --start-period=5s --retries=3 CMD [ "curl", "--fail", "localhost:8123/healt", "||", "kill", "1" ]